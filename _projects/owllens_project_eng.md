---
layout: page
title: 🦉OwlLens - Intelligent Image Retrieval Assistant
description: A brand new offline AI image search experience that makes your photo management smarter and more secure
img: assets/img/owlLens/menu.png
importance: 1
category: app
---


<div class="row justify-content-center">
    <div class="col-sm-8 mt-3 mt-md-0">
        <a href="https://apps.apple.com/us/app/owllens/id6746360832" target="_blank" class="btn btn-primary btn-lg btn-block" style="background-color: #007AFF; border-color: #007AFF;">
            <i class="fab fa-app-store-ios"></i> Download on the Mac App Store
        </a>
    </div>
</div>

## 🌟 Product Highlights

Discover your photos like never before! OwlLens is a powerful offline image retrieval tool that makes it easy to manage and find your photos.

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/owlLens_eng/1.png" title="OwlLens Main Interface" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    A clean and elegant user interface design for a smooth user experience
</div>

### ✨ Core Features

<div class="l-screen-inset">
    <div class="row">
        <div class="col-sm mt-3 mt-md-0">
            {% include figure.html path="assets/img/owlLens_eng/4.png" title="Natural Language Search" class="img-fluid rounded z-depth-1" %}
        </div>
        <div class="col-sm mt-3 mt-md-0">
            {% include figure.html path="assets/img/owlLens_eng/5.png" title="Search by Image" class="img-fluid rounded z-depth-1" %}
        </div>
        <div class="col-sm mt-3 mt-md-0">
            {% include figure.html path="assets/img/owlLens_eng/6.png" title="batch-image-search" class="img-fluid rounded z-depth-1" %}
        </div>
    </div>
</div>
<div class="caption text-center">
    Left: Quickly find images using natural language descriptions; Middle: Search for similar photos with an example image; Right: Supports batch image retrieval
</div>

- **Intelligent Understanding**: Advanced AI models comprehend your search intent
- **Flexible Queries**: Search using everyday language, no need for specific keywords
- **Visual Similarity**: Find photos with similar visual elements
- **Efficient Management**: Process multiple images simultaneously

### 🔒 Privacy First
- **Fully offline**: Built-in AI models, no internet required
- **Local processing**: All operations are performed locally to ensure data security
- **No cloud storage**: Your photos always stay on your own device

### 📥 Flexible Import
- **Multiple import methods**: Supports importing from folders
- **System album integration**: Seamlessly connects to your system photo library
- **Batch processing**: Easily manage large numbers of images

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/owlLens_eng/7.png" title="Import Demo" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    Simple and intuitive import interface to quickly start your photo management journey
</div>

### 🍎 Native Mac Experience
- Native interface design
- Fast response
- Excellent performance
- Simple and elegant user experience

## 📞 Contact
- Email: jiajiageng@outlook.com